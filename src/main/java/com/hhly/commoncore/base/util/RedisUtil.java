...
public class RedisUtil {
   ...
   */  
	 ...
	 */
	@SuppressWarnings("unchecked")
	public <T> T popObj(String key, Class<T> classt) {
		Object popResult = null;
		try {
			popResult = redisTemplate.opsForList().leftPop(key);
		} catch (Exception e) {
			logger.warn(spellString("pop {0}", key), e);
		}
		return popResult == null ? null : (T) popResult;
	}
